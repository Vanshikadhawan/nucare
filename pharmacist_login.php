            ...
            if($rowcount==1)
            {
                if($row=mysqli_fetch_array($result))
                {
                        if(decrypt($row['password'])== $pass)
                        {
                            ...
                            $_SESSION['pname']=$row['pharmacist_name'];
                    
                          header('location:pharmacist_welcome.php');
                        }
                        else
                        {
                            header('location:pharmacist_wrong.php');
                        }
    			...
    			<form id="f1" name="f1" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" >
                    ...
                    <div class="form-group row">
                        <div class="col-sm-12 text-center">
                            ...
                            <a href="Pharmacist_register.php" > New User Sign Up Here </a>
                        </div>
                    </div>
    			</form>
            </div>
